...
export const vendorSignin = async (req, res, next) => {
  ...
  try {
    ...
    if (!validVendor || !validVendor.isVendor) return next(errorHandler(404, "user not found"));
...
export const vendorGoogle = async (req, res, next) => {
  try {
    ...
    if (user && !user.isVendor) return next(errorHandler(409, "email already in use as a user"));
    ...
    if (user && user.isVendor) {
      ...
      const { password: hashedPassword, ...rest } = updatedVendor._doc;
      ...
      try{
        ...
        const { password: hashedPassword2, ...rest } = updatedVendor._doc;
        res.status(200).json({ ...rest, accessToken, refreshToken });
      }
      catch(error){
        if(error.code === 11000){
