    ...
    const generateMatchStage = (data) => {
      ...
      data.forEach((cur) => {
        ...
        if (cur.type === "car_type") {
          carTypes.push(key);
        } else if (cur.type === "transmition") {
          transmitions.push(key);
        } else if (cur.type === "fuel_type") {
          // Map 'ev' from frontend to 'electirc' if that's what the DB uses
          fuelTypes.push(key === "ev" ? "electirc" : key);
        } else if (cur.type === "seats") {
          // Map 'seats_4' -> 4, etc.
          const seatCount = parseInt(key.replace("seats_", ""));
          if (!isNaN(seatCount)) seats.push(seatCount);
        }
      });

      const conditions = [];
      if (carTypes.length > 0) conditions.push({ car_type: { $in: carTypes } });
      if (transmitions.length > 0) conditions.push({ transmition: { $in: transmitions } });
      if (fuelTypes.length > 0) conditions.push({ fuel_type: { $in: fuelTypes } });
      if (seats.length > 0) conditions.push({ seats: { $in: seats } });
...
export const cancelBooking = async (req, res, next) => {
  try {
    ...
    if (booking.status === "canceled") {
       return next(errorHandler(400, "Booking already canceled"));
    }
    ...
    console.log(error);
    next(errorHandler(500, "Error canceling booking"));
  }
};