...
export async function displayRazorpay(values, navigate, dispatch) {
  try {
    ...
    // FALLBACK: If Razorpay fails (common in dev), proceed with mock booking
    if (!data.ok) {
      toast.info("Proceeding with test booking (Developer Mode)");
      const mockDbData = { 
        ...values, 
      ...
      const resBook = await fetch("/api/user/bookCar", {
        method: "POST",
    ...
    const options = {
      ...
      prefill: {
        name: values.customerDetails?.username || "Guest",
        email: values.customerDetails?.email || "",
        contact: values.customerDetails?.phoneNumber || "",
      },
      theme: {
        color: "#61dafb",
      },
